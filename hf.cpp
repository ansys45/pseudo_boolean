...
{   
            ...
            while (!queue.empty()) {
                ...
                for (auto c : s->children) {
                    if (visited.find(c) == visited.end()) {
                        ...
                        // vector <long double> l(FUNC[1].size() + 1, 0);
    ...
    for (int i = 1; i < violations.size(); i++) {
        if (violations[i] <= 0 && solution[i] == 2) {
            solution[i] = 1;
            return branch(solution);
        }
...
{   
    ...
    vector<int> answers = {38, 932615, 977799, 1010641, 1034976, 796648, 854704, 893782, 928941, 793439, 851495, 893076, 928941};
